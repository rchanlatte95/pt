...
namespace rac::mth
{
    ...
    v3::mut_v3(f32 x_, f32 y_)
    {
        x = x_;
        y = y_;
        z = 0.0f;
    }
    v3::mut_v3(f32 x_, f32 y_, f32 z_)
    {
        x = x_;
        y = y_;
        z = z_;
    }
    v3::mut_v3(v2 v, f32 z_)
    {
        x = v.x;
        y = v.y;
        z = z_;
    ...
    INLINE v3 v3::operator-() const noexcept { return v3(-x, -y, -z); }
    INLINE f32 v3::operator[](i32 i) const noexcept { return *(&x + i); }
    INLINE mut_f32ref v3::operator[](i32 i) { return *(&x + i); }

    INLINE mut_v3_ref v3::operator=(v3_ref v_ref) noexcept
    {
        x = v_ref.x;
        y = v_ref.y;
        z = v_ref.z;
    ...
    INLINE mut_v3_ref v3::operator+=(v3_ref v) noexcept
    {
        x += v.x;
        y += v.y;
        z += v.z;
    ...
    INLINE mut_v3_ref v3::operator*=(f32 v) noexcept
    {
        x *= v;
        y *= v;
        z *= v;
    ...
    INLINE f32 v3::SqrMag() const noexcept
    {
        return x * x + y * y + z * z;
    }
    INLINE f32 v3::InvSqrMag() const noexcept
    {
        return 1.0f / (x * x + y * y + z * z);
    }
    INLINE f32 v3::Mag() const noexcept
    {
        return sqrtf(this->SqrMag());
    }
    INLINE f32 v3::InvMag() const noexcept
    {
        return 1.0f / sqrtf(this->SqrMag());
    }
    INLINE v3 v3::Norm() const noexcept
    {
        f32 invMag = this->InvMag();
        return v3(x * invMag, y * invMag, z * invMag);
    }

    INLINE pf128 v3::Pack128() const noexcept
    {
        return pf128(x, y, z, 0.0f);
    }

    INLINE static_strings::mut_cachestr v3::ToStr(u32 decimal_places) const noexcept
    {
