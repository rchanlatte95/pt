...
namespace rac::mth
{
    ...
    mut_v2::mut_v2(f32 x_, f32 y_)
    {
        x = x_;
        y = y_;
    }

    INLINE v2 mut_v2::operator-() const noexcept { return mut_v2(-x, -y); }
    INLINE f32 mut_v2::operator[](i32 i) const noexcept { return *(&x + i); }
    INLINE mut_f32ref mut_v2::operator[](i32 i) { return *(&x + i); }

    INLINE v2_ref mut_v2::operator+=(const mut_v2 v) noexcept
    {
        x += v.x;
        y += v.y;
    }
    ...
    INLINE v2_ref mut_v2::operator*=(f32 v) noexcept
    {
        x *= v;
        y *= v;
    }
    ...
    INLINE f32 mut_v2::SqrMag() const noexcept { return x * x + y * y; }
    INLINE f32 mut_v2::InvSqrMag() const noexcept
    {
        return 1.0f / (x * x + y * y);
    }
    INLINE f32 mut_v2::Mag() const noexcept
    {
        return sqrtf(this->SqrMag());
    }
    INLINE f32 mut_v2::InvMag() const noexcept
    {
        return 1.0f / sqrtf(this->SqrMag());
    }
    INLINE v2 mut_v2::Norm() const noexcept
    {
